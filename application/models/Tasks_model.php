...
class Tasks_model extends CRM_Model
{
    ...
    public function get_home_tasks($limit = 15){
        $where = '(id IN (SELECT taskid FROM tblstafftaskassignees WHERE staffid = ' . get_staff_user_id() . ')';
    ...
    public function get_detailed_overview(){
        $overview = array();
        $staff_id = ($this->input->post('member') || $this->input->post('member') == '' ? $this->input->post('member') : get_staff_user_id());
