...
class Contracts_model extends CRM_Model
{
    ...
    /**
    * Add new contract equipment
    * @since  Version 1.0.3
    * @param mixed $data All $_POST data
    */
   public function add_contract_equipment($data)
   {
      if (isset($data['custom_fields'])) {
          $custom_fields = $data['custom_fields'];
          unset($data['custom_fields']);
      }
      $this->db->insert('tblcontractequipments', $data);
      $insert_id = $this->db->insert_id();
      if ($insert_id) {
          if (isset($custom_fields)) {
             handle_custom_fields_post($insert_id, $custom_fields);
          }
          logActivity('New Contract Equipment Added [' . $data['name'] . ']');
          return $insert_id;
      }
      return false;
   }
   /**
    * Edit contract equipment
    * @since  Version 1.0.3
    * @param mixed $data All $_POST data
    * @param mixed $id Contract equipment id
    */
   public function update_contract_equipment($data, $id)
   {
      $affectedRows = 0;
      if (isset($data['custom_fields'])) {
          $custom_fields = $data['custom_fields'];
      ...
      $this->db->where('id', $id);
      $this->db->update('tblcontractequipments', $data);
      if ($this->db->affected_rows() > 0) {
          logActivity('Contract Equipment Updated [' . $data['subject'] . ']');
          return true;
      }
      if ($affectedRows > 0) {
          return true;
    ...
    * @param  integer ID
    * @return mixed
    * Delete contract equipment from database
    */
   public function delete_contract_equipment($id)
   {
      $this->db->where('id', $id);
      $this->db->delete('tblcontractequipments');
      if ($this->db->affected_rows() > 0) {
          // Delete the custom field values
          $this->db->where('relid', $id);
          $this->db->where('fieldto', 'equipment');
          $this->db->delete('tblcustomfieldsvalues');

          logActivity('Contract Equipment Deleted [' . $id . ']');
          return true;
      }
      return false;
   }
   public function get_contract_id_with_equipment($id='')
   {
      if (is_numeric($id)) {
         $this->db->where('id', $id);
         return $this->db->get('tblcontractequipments')->row()->contractid;
      }
   }
}
