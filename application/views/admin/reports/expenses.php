            ...
            <div class="col-md-12">
                <div class="panel_s">
                    <div class="panel-body">
                      <h4 class="no-margin pull-right text-danger" data-toggle="tooltip" data-placement="left" title="<?php echo _l('expense_report_info'); ?>"><i class="fa fa-exclamation-triangle"></i></h4>
                        ...
                        <select class="selectpicker" name="currencies" onchange="change_expense_currency(this.value);"  data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" >
                                    ...
                                    <tr>
                                        ...
                                        <?php
                                            ...
                                            for ($m=1; $m<=12; $m++) {
                                                ...
                                                // Ge the expenses
                                                $this->db->select('amount,taxrate,tblexpenses.tax')->from('tblexpenses')->join('tbltaxes','tbltaxes.id = tblexpenses.tax','left')->where('MONTH(date)',$m)->where('YEAR(date)',$current_year)->where('category',$category['id'])->where('billable',0)->where('currency',$_currency->id);
...
<script>
    ...
    function change_expense_currency(id){
        var year = $('select[name="expense_year"]').val();
        window.location.href = admin_url + 'reports/expenses/' + year+'?currency='+id;
    }
