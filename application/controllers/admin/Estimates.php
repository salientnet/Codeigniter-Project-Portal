...
class Estimates extends Admin_controller
{
    ...
    public function list_estimates($id = '', $clientid = false)
    {
        ...
        if ($this->session->has_userdata('estimate_pipeline') && $this->session->userdata('estimate_pipeline') == 'true' && $clientid == false) {
    ...
    {
        if (has_permission('estimates', '', 'view')) {
            $data['statuses'] = $this->estimates_model->get_statuses();
