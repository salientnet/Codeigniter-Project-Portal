...
class Surveys extends Admin_controller
{
            ...
            foreach ($lists as $key => $val) {
                ...
                if (is_int($key)) {
                    ...
                    if ($key == 'staff') {
                        ...
                        if (count($staff) > 0) {
                            array_push($_lists, l('survey_send_mail_list_staff'));
                        }
                    } else if ($key == 'clients') {
    ...
    public function remove_question($questionid)
    {
        ...
        if ($this->input->is_ajax_request()) {
           $this->surveys_model->remove_question($questionid);
        }
    }
    ...
    public function remove_box_description($questionboxdescriptionid)
    {
        ...
        if ($this->input->is_ajax_request()) {
            $this->surveys_model->remove_box_description($questionboxdescriptionid);
        }
    }
    /* Add box description */
